...
class MenuManager {
  ...
  renderCategories() {
    ...
    const categoryItems = this.categories.map((category) => {
      ...
      return this.createCategoryItem(category, dishCount, this.currentCategory === category.idDishcategory)
    })
  ...
  createCategoryItem(category, count, isActive) {
    return `
      ...
      <div class="dish-card ${isListView ? "list-view" : ""} ${dish.issoldout ? "soldout" : ""}" 
        ...
        <div class="dish-content">
          ...
          <div class="dish-actions">
            <button class="btn btn-view-detail" data-dish-id="${dish.idDish}">
  ...
  addToCart(dishId) {
    ...
    if (window.addToCart) {
      ...
      this.showNotification("success", "Thành công", `Đã thêm "${dish.name}" vào giỏ hàng!`)
    } else {
  ...
  showNotification(type, title, message) {
    if (window.showModernNotification) {
      window.showModernNotification(type, title, message)
    }
  }
}
