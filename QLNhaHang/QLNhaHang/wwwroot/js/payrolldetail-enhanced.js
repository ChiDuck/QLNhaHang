class PayrollDetailManager {
  ...
  applyFilters() {
    ...
    this.filteredData = this.payrollData.filter((employee) => {
      ...
      const matchesDepartment = !departmentFilter || employee.type === departmentFilter
  ...
  // Salary Calculation
  calculateSalary() {
    const days = Number.parseInt(document.getElementById("pd_Days").value) || 0
    ...
    // Basic salary calculation (this should be based on your business logic)
    const baseSalaryPerDay = 300000 // 300k per day
    const overtimeRate = 50000 // 50k per hour overtime
    const absencePenalty = 100000 // 100k per absence
    const latePenalty = 50000 // 50k per late

    let totalSalary = days * baseSalaryPerDay

    // Add overtime (assuming standard 8 hours per day)
    const standardHours = days * 8
    const overtimeHours = Math.max(0, hours - standardHours)
    totalSalary += overtimeHours * overtimeRate

    // Subtract penalties
    totalSalary -= absences * absencePenalty
    totalSalary -= lates * latePenalty
