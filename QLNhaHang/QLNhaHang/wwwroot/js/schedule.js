class ScheduleManager {
    ...
    applyScheduleFilter() {
        ...
        console.log("Applying filter for staff:", input, "Found:", found)
        if (!found) {
        ...
        console.log("Highlighting schedule for staff ID:", selectedId)
    ...
    async showShiftDetail(day, shift, isNew) {
        ...
        try {
            ...
            await this.loadAvailableStaff(data.staff.map((s) => s.idStaff))
                ...
                <td>
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <small class="text-muted">ID: ${st.idStaff}</small>
                        </div>
                    </div>
                </td>
                <td>
                    <span class="badge bg-secondary">${st.staffType}</span>
                </td>
                <td class="text-center">
                    <div class="form-check form-switch d-flex justify-content-center">
                        <input class="form-check-input" type="checkbox"
                               ...
                               onchange="window.scheduleManager.toggleAttendance(${st.idStaff}, this.checked)">
                    </div>
                </td>
                <td class="text-center">
                    <div class="form-check form-switch d-flex justify-content-center">
                        <input class="form-check-input late-checkbox" type="checkbox"
                               ...
                               onchange="window.scheduleManager.toggleLate(${st.idStaff}, this.checked)">
                    </div>
                </td>
                <td class="text-center">
                    <button class="btn btn-sm btn-outline-danger" onclick="window.scheduleManager.removeStaff(${st.idStaff})" title="Xóa khỏi ca">
    ...
    updateStaffSearchResults() {
        ...
        const filteredStaff = this.allStaffOptions
            .filter((opt) => opt.name.toLowerCase().includes(keyword) && !this.selectedStaffIds.includes(opt.idStaff))
        ...
        filteredStaff.forEach((opt) => {
            ...
            btn.innerHTML = `
                ...
                <div>
                    ...
                    <small class="text-muted">${opt.staffType}</small>
                </div>
            `
            btn.onclick = () => {
                this.selectedStaffIds.push(opt.idStaff)
    ...
    updateSelectedPreview() {
        ...
        this.selectedStaffIds.forEach((idStaff) => {
            const opt = this.allStaffOptions.find((s) => s.idStaff === idStaff)
            if (!opt) return

            const div = document.createElement("div")
            div.className = "badge bg-primary me-2 mb-2 p-2"
            div.innerHTML = `
                ${opt.name} (${opt.staffType}) 
                <span class="ms-2" style="cursor:pointer;" onclick="window.scheduleManager.removeSelectedStaff(${idStaff})" title="Xóa">
    ...
    removeSelectedStaff(id) {
        this.selectedStaffIds = this.selectedStaffIds.filter((idStaff) => idStaff !== id)
        this.updateSelectedPreview()
        this.updateStaffSearchResults()
    }
...
window.removeStaff = (idStaff) => {
    window.scheduleManager.removeStaff(idStaff)
}
